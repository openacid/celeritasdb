...
pub fn test_base_trait(eng: &dyn Base) {
    let none = eng.next(DBColumnFamily::Default, &"init".as_bytes().to_vec(), true);
    assert_eq!(none, None);
    let none = eng.next(DBColumnFamily::Instance, &"init".as_bytes().to_vec(), true);
    assert_eq!(none, None);

    let none = eng.prev(DBColumnFamily::Default, &"init".as_bytes().to_vec(), true);
    assert_eq!(none, None);
    ...
    let none = eng
        .get(DBColumnFamily::Default, &"init".as_bytes().to_vec())
        .unwrap();
    assert_eq!(none, None);
    let none = eng
    ...
    let r = eng.delete(DBColumnFamily::Default, &"init".as_bytes().to_vec());
    assert!(r.is_ok());
    ...
    let r = eng.get(DBColumnFamily::Default, &kvs[0].0).unwrap();
    assert_eq!(None, r);
    let r = eng.get(DBColumnFamily::Status, &kvs[0].0).unwrap();
    assert_eq!(r, Some(kvs[0].1.clone()));

    let next = eng.next(DBColumnFamily::Default, &kvs[0].0, true);
    assert!(next.is_none());
    ...
    let prev = eng.prev(DBColumnFamily::Default, &kvs[0].0, true);
    assert!(prev.is_none());
    ...
    eng.delete(DBColumnFamily::Default, &kvs[0].0).unwrap();
    ...
    assert_eq!(
        v1.clone(),
        eng.get(DBColumnFamily::Default, &k1).unwrap().unwrap()
    );
    assert_eq!(
        v2.clone(),
    ...
    eng.write_batch(&cmds).unwrap();
    assert_eq!(None, eng.get(DBColumnFamily::Default, &k1).unwrap());
}
