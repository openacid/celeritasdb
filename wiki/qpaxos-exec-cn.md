...
这个算法也适用于不使用 seq 的场景, 只需要实现满足一下条件:
...
- `C₂(X)`: 删掉`C₁(X)`中从`V₁(X)`出发的边后, 沿着**min-edge**遍历发现的第 2 个环。
  ...

  `{C₁}`: 所有第 1 轮中找到的环的集合: `{C₁(X) | X ∈ G}`.
  `{C₂}`: 所有第 2 轮中找到的环的集合: `{C₂(X) | X ∈ G}`.
  ...

- `Vᵢ(X)` 的定义跟 `V₁(X)` 类似.

  `{Vᵢ}`: 所有在第`i`轮找到的关键 vertex:
  `{Vᵢ(X) | X ∈ G}`.
...
在执行 exec 算法之前, 删除一个不在任何`Pk(X)`中的边不会影响执行结果。
...
在一个 SCC 中, 对每一个`Vᵢ`中的节点, `X: X ∈ {Vᵢ}, i > 0`,
...
1.  如果 `Cᵢ` 不包含任何`U: U ∈ {Vⱼ}, 0<j<i`:
    ...
    ∴ 存在一个 key path `Pk` 包含边 `(A, X)`, 这里`A: A ∈ {V₀}`.

2.  如果 `Cᵢ` 包含一个`{Vᵢ}`节点 U: `U: U ∈ {Vⱼ}, 0<j<i`:
    ...
    ∴ `(B, X)` 存在于某个 key path `Pk`中.

∴ 通过 1 和 2, 对任意 `i>0`,
总能找到一个 key path `Pk`, 使得对每个 `Vᵢ` vertex 都有一个 `V₀` vertex 在它之前.
...
后续的遍历都会找到相同的 instance 去执行.

∴ `X` 和 `Y` 在不同的 replcia 上总是有相同的执行顺序.
...
一个顶点的`seq`有 50%的概率是`min(Seqs(X))`, 50%是`max(Seqs(X))`
...
`C(m, n) = m!/n!/(m-n)!` 是从 m 个中选择 n 个的组合数。
