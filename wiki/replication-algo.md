...
For instance `a`, `a.deps` is a set of **instance-ids** that **should** be
executed before `a`.
...
Finaly `a.committedDeps = {b}`, `b.committedDeps = {a}`.

For this reason execution order is determined not by relation `>`, but by
comparing the vecotr `a.committedDeps`.

TODO

## Guarantees:

- Execution consistency: on every replica.

- Execution linearizability: If two instance

## For interfering instances:

- `a.committedDeps ⊃ b.committedDeps` : exec `a` after `b`
- `a.committedDeps ⊅ b.committedDeps` and `a.committedDeps ⊄ b.committedDeps` :
  exec `a` and `b` in instance id order.
- there is no `a` and `b` have `a.committedDeps == b.committedDeps`.

## For non-interfering instances:

- `a.committedDeps ⊃ b.committedDeps` : exec `a` after `b`
- `a.committedDeps == b.committedDeps`: exec `a` and `b` in instance id order.
- `a.committedDeps ⊅ b.committedDeps` and `a.committedDeps ⊄ b.committedDeps` :
  exec `a` and `b` in instance id order.
